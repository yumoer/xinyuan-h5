    ...
    <div class="content container">
      ...
      <div class="arrow" style="margin-top: 26px">
        <el-input placeholder="职位名称" v-model="input" class="input-with-select" clearable>
          ...
          <el-button slot="append" @click="search" style="width: 100%;height: 100%">搜索</el-button>
        </el-input>
      </div>
      <el-card class="box-card" >
        ...
        <el-collapse v-model="activeName" accordion v-for="(navigation,index) in this.arrayDate" :key="index">
          <el-collapse-item>
            <template slot="title">
              <div class="row">
...
<script>
  ...
  import {reqNavigationBar} from '../../api'
  ...
  export default {
    ...
    methods: {
      ...
      async search(){
        const resultData = await reqNavigationBar({jobName:this.input,page:1,limit:this.pagesize})
        this.total = resultData.count
        this.arrayDate = resultData.data;
        /*for (let i=0;i<this.arrayDate.length;i++){
          //for循环数据中的每一项（根据name值）

          if(this.arrayDate[i].jobName.search(this.input) != -1){
            //判断输入框中的值是否可以匹配到数据，如果匹配成功
            this.arrayDate.push(this.arrayDate[i]);
      ...
      async handleCurrentChange(currentPage) {
        ...
        const result = await reqNavigationBar({jobName:this.input,page:this.currentPage,limit:this.pagesize})
        this.arrayDate = result.data
      },
    },
    computed:{
    ...
    async mounted(){
      ...
      // this.$store.dispatch('getNavigationBar',{jobName:this.input,page:this.currentPage,limit:this.pagesize})
    },
  }
</script>
